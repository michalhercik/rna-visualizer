...
export default function zoom(data: RNAData) {
    const margin = 10;
    const zoom = d3.zoom()
    .scaleExtent([1,10])
    .on('zoom', update);
    ...
    function updateLabels(event: any): void {
        ...
        .attr('x', (label: Label) => x(formCoor(label.labelContent.x)))
        .attr('y', (label: Label) => y(formCoor(label.labelContent.y)))
        .style('font-size', getFontSize(data.classes) * 0.75 * event.transform.k);
